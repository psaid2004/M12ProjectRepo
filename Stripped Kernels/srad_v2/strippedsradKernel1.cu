...
namespace sradv2_1 {
    ...
    static float q0sqr, sum, sum2, tmp, meanROI,varROI ;
...
int launcher(int partitions, cudaStream_t stream) 
{
	...
	for (int i = 0; i < partitions; i ++){
		...
		//Run kernels
		srad_cuda_1<<<dimGrid, dimBlock,0, stream>>>(d_E_C + offset, d_W_C + offset, d_N_C + offset, d_S_C + offset, d_J + offset, d_C + offset, cols, rows/partitions, q0sqr, chunkSize); 
	...
	CUDA_CHECK(cudaMemcpyAsync(h_C , d_C , sizeof(float) * cols * rows, cudaMemcpyDeviceToHost, stream));
	CUDA_CHECK(cudaMemcpyAsync(h_E_C, d_E_C, sizeof(float) * cols * rows, cudaMemcpyDeviceToHost, stream));
	CUDA_CHECK(cudaMemcpyAsync(h_W_C , d_W_C , sizeof(float) * cols * rows, cudaMemcpyDeviceToHost, stream));
	CUDA_CHECK(cudaMemcpyAsync(h_N_C , d_N_C, sizeof(float) * cols * rows, cudaMemcpyDeviceToHost, stream));
	CUDA_CHECK(cudaMemcpyAsync(h_S_C, d_S_C, sizeof(float) * cols * rows, cudaMemcpyDeviceToHost, stream));
