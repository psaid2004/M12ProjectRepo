...
runTest( int argc, char** argv) 
{
	...
	CUDA_CHECK(cudaMemcpy(d_C, h_C , sizeof(float) * cols*rows, cudaMemcpyHostToDevice));
	CUDA_CHECK(cudaMemcpy(d_E_C, h_E_C  , sizeof(float) * cols*rows, cudaMemcpyHostToDevice));
	CUDA_CHECK(cudaMemcpy(d_W_C, h_W_C  , sizeof(float) * cols*rows, cudaMemcpyHostToDevice));
	CUDA_CHECK(cudaMemcpy(d_N_C, h_N_C , sizeof(float) * cols*rows, cudaMemcpyHostToDevice));
	CUDA_CHECK(cudaMemcpy(d_S_C, h_S_C , sizeof(float) * cols*rows, cudaMemcpyHostToDevice));
	...
	for (int i = 0; i < NUM_OF_PARTITIONS; i ++){
		...
		srad_cuda_2<<<dimGrid, dimBlock, 0>>>(d_E_C + offset, d_W_C + offset, d_N_C + offset, d_S_C + offset, d_J + offset, d_C + offset, cols, rows/NUM_OF_PARTITIONS, lambda, q0sqr, chunkSize);
