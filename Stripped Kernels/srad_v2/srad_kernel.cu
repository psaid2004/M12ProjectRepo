...
) 
{
  ...
  //indices
  int index   = rows * BLOCK_SIZE * by + BLOCK_SIZE * bx + cols * ty + tx;
  int index_n = rows * BLOCK_SIZE * by + BLOCK_SIZE * bx + tx - cols;
  int index_s = rows * BLOCK_SIZE * by + BLOCK_SIZE * bx + cols * BLOCK_SIZE + tx;
  int index_w = rows * BLOCK_SIZE * by + BLOCK_SIZE * bx + cols * ty - 1;
  int index_e = rows * BLOCK_SIZE * by + BLOCK_SIZE * bx + cols * ty + BLOCK_SIZE;
  ...
  //Change Values so partitioning works(NOT MATHEMATICALLY CORRECT)
  index_n = (index_n < 0)          ? index : index_n;
  index_s = (index_s >= chunkSize) ? index : index_s;
  index_w = (index_w < 0)          ? index : index_w;
  index_e = (index_e >= chunkSize) ? index : index_e;
