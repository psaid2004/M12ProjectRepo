...
runTest( int argc, char** argv) 
{
    ...
    CUDA_CHECK(cudaMemcpy(d_J, h_J, sizeof(float) * cols*rows, cudaMemcpyHostToDevice));
	...
	for (int i = 0; i < NUM_OF_PARTITIONS; i ++){
		...
		//Run kernels
		srad_cuda_1<<<dimGrid, dimBlock,0>>>(d_E_C + offset, d_W_C + offset, d_N_C + offset, d_S_C + offset, d_J + offset, d_C + offset, cols, rows/NUM_OF_PARTITIONS, q0sqr, chunkSize); 
    ...
    CUDA_CHECK(cudaMemcpy(h_J, d_J, sizeof(float) * cols*rows, cudaMemcpyDeviceToHost));
	CUDA_CHECK(cudaMemcpy(h_C, d_C , sizeof(float) * cols*rows, cudaMemcpyDeviceToHost));
	CUDA_CHECK(cudaMemcpy(h_E_C, d_E_C  , sizeof(float) * cols*rows, cudaMemcpyDeviceToHost));
	CUDA_CHECK(cudaMemcpy(h_W_C, d_W_C  , sizeof(float) * cols*rows, cudaMemcpyDeviceToHost));
	CUDA_CHECK(cudaMemcpy(h_N_C, d_N_C , sizeof(float) * cols*rows, cudaMemcpyDeviceToHost));
	CUDA_CHECK(cudaMemcpy(h_S_C, d_S_C , sizeof(float) * cols*rows, cudaMemcpyDeviceToHost));
