...
int main(int argc, char **argv)
{
    ...
    for(int i = 0; i <NUM_OF_PARTITIONS; i++){
        ...
        cudaMemcpyAsync(d_temp[0] + offset, h_temp + offset,  chunkSize * sizeof(float), cudaMemcpyHostToDevice, streams[i]);
        cudaMemcpyAsync(d_temp[1] + offset, h_temp + offset,  chunkSize * sizeof(float), cudaMemcpyHostToDevice, streams[i]);

        cudaMemcpyAsync(d_power + offset, h_power + offset, chunkSize * sizeof(float), cudaMemcpyHostToDevice, streams[i]);
    ...
    for (int t = 0; t < total_iterations; t += pyramid_height) {
        ...
        calculate_temp<<<dimGrid, dimBlock, 0, streams[i]>>>(
    ...
    // After loop, copy final result out once
    for(int i = 0; i < NUM_OF_PARTITIONS; i++){
        int offset = i * chunkSize;
        cudaMemcpyAsync(h_out + offset, d_temp[src] + offset, chunkSize * sizeof(float), cudaMemcpyDeviceToHost, streams[i]);
    }
