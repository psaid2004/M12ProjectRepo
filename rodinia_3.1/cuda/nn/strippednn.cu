...
int main(int argc, char* argv[])
{
	...
	*/
    ...
    int chunkSize = numRecords/NUM_OF_PARTITIONS;

    cudaStream_t streams[NUM_OF_PARTITIONS]; 
    for(int i=0; i < NUM_OF_PARTITIONS; i++){
        cudaStreamCreate(&streams[i]);
        int offset = i *chunkSize;
        cudaMemcpyAsync(d_locations+offset, &locations[0]+offset, sizeof(LatLong) * chunkSize, cudaMemcpyHostToDevice,streams[i]);

        /**
        * Execute kernel
        */
        euclid<<< gridDim, threadsPerBlock,0, streams[i] >>>(d_locations+offset,d_distances+offset,numRecords,lat,lng);
