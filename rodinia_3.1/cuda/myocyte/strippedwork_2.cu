...
int work_2(	int xmax,
	...
	threads.y = 1;
	blocks.x = 8/NUM_OF_PARTITIONS;																				// define the number of blocks in the grid
	blocks.y = 1;
	int chunkSize = (threads.x * (blocks.x));
    int chunkSize_x = chunkSize * (xmax+1);
    int chunkSize_y = chunkSize * (xmax+1) * EQUATIONS;
    int chunkSize_params = chunkSize * PARAMETERS;
    int chunkSize_com = chunkSize * 3;
    int chunkSize_err = chunkSize * EQUATIONS;

    // Could not make it work with partitioning memory
	...
	for(i = 0; i < NUM_OF_PARTITIONS; i++) {
        int tid_offset = i *chunkSize;
		int offset_x = i * chunkSize_x;
        int offset_y = i * chunkSize_y;
        int offset_params = i * chunkSize_params;
        int offset_com = i * chunkSize_com;
        int offset_err = i * chunkSize_err;
		cudaStreamCreate(&streams[i]);

		// cudaMemcpyAsync(d_x + offset_x, x + offset_x, chunkSize_x* sizeof(fp), cudaMemcpyHostToDevice, streams[i]);

		// cudaMemcpyAsync(d_y + offset_y, y + offset_y, chunkSize_y * sizeof(fp), cudaMemcpyHostToDevice, streams[i]);
		// cudaMemcpyAsync(d_params+offset_params, params+offset_params, chunkSize_params*sizeof(fp), cudaMemcpyHostToDevice, streams[i]);

		solver_2<<<blocks, threads, 0, streams[i]>>>(	workload,
																xmax,

																d_x + offset_x,
																d_y + offset_y,
																d_params + offset_params,
																d_com + offset_com,
																d_err + offset_err,
																d_scale + offset_err,
																d_yy + offset_err,
																d_initvalu_temp + offset_err,
																d_finavalu_temp + i*chunkSize*13*EQUATIONS, tid_offset);
		// cudaMemcpyAsync(x+offset_x, d_x+offset_x, chunkSize_x* sizeof(fp), cudaMemcpyDeviceToHost, streams[i]);
		// cudaMemcpyAsync(y+offset_y, d_y+offset_y, chunkSize_y * sizeof(fp), cudaMemcpyDeviceToHost, streams[i]);
	}
