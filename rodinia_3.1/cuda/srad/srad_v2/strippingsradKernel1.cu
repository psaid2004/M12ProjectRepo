...
runTest( int argc, char** argv) 
{
    ...
    float *h_I, *h_J, q0sqr, sum, sum2, tmp, meanROI,varROI ;
	...
	// int partitioned_rows = row_offset/NUM_OF_PARTITIONS;
	...
	for (int i = 0; i < NUM_OF_PARTITIONS; i ++){
		...
		//should q0sqr be copied over as well?

		//Run kernels
		srad_cuda_1<<<dimGrid, dimBlock,0, streams[i]>>>(d_E_C + offset, d_W_C + offset, d_N_C + offset, d_S_C + offset, d_J + offset, d_C + offset, cols, rows/NUM_OF_PARTITIONS, q0sqr, chunkSize); 


		//Copy data from device memory to main memory
		 CUDA_CHECK(cudaMemcpyAsync(h_J + offset, d_J + offset, sizeof(float) * chunkSize, cudaMemcpyDeviceToHost, streams[i]));
		 CUDA_CHECK(cudaMemcpyAsync(h_C + offset, d_C + offset, sizeof(float) * chunkSize, cudaMemcpyDeviceToHost, streams[i]));
		 CUDA_CHECK(cudaMemcpyAsync(h_E_C + offset, d_E_C + offset, sizeof(float) * chunkSize, cudaMemcpyDeviceToHost, streams[i]));
		 CUDA_CHECK(cudaMemcpyAsync(h_W_C + offset, d_W_C + offset, sizeof(float) * chunkSize, cudaMemcpyDeviceToHost, streams[i]));
		 CUDA_CHECK(cudaMemcpyAsync(h_N_C + offset, d_N_C + offset, sizeof(float) * chunkSize, cudaMemcpyDeviceToHost, streams[i]));
		 CUDA_CHECK(cudaMemcpyAsync(h_S_C + offset, d_S_C + offset, sizeof(float) * chunkSize, cudaMemcpyDeviceToHost, streams[i]));
